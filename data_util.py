...
def load_file(filepath):
    col = ["unit_num", "time_cycles", "setting_1", "setting_2",
           "setting_3"] + [f"senor_{i}" for i in range(1, 22)]
    df = pd.read_csv(filepath, sep="\s+", header=None, names=col)
    df = df.drop(columns=["setting_1", "setting_2", "setting_3"] +
                 [f"senor_{i}" for i in (1, 5, 6, 10, 16, 18, 19)])
    return df


def load_RUL(filepath):
    return pd.read_csv(filepath, sep='\s+', header=None, names=['RUL']).to_numpy()
...
class TrainLoader(Dataset):
    ...
    def norm(self, data, title):
        ...
        result_frame = train_norm.merge(max_cycle.to_frame(
            name='max_cycle'), left_on='unit_num', right_index=True)
